              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-slate-50">
                    <tr>
                      {['Rank','Agent','Calls Today','Calls This Week','Calls This Month','Conversions','Target',
                        ...(isAdmin ? ['Earnings'] : [])
                      ].map(h => (
                      ...
                      return (
                        <tr key={agent.agent_id} className={`hover:bg-slate-50 ${isMe ? 'bg-indigo-50' : ''}`}>
                          <td className="px-4 py-3">
                            <span className="text-lg">{MEDALS[agent.rank-1] || `#${agent.rank}`}</span>
                          </td>
                          <td className="px-4 py-3">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <div className="w-16 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                                <div className="h-full rounded-full"
                                  style={{ width:`${p}%`, background: p>=100?'#16a34a':p>=70?'#d97706':'#4f46e5' }} />
                              </div>
                            </div>
                          </td>
            ...
            <div className="card p-5">
              <div className="flex items-center justify-between mb-5">
                <div>
                  ...
                  <p className="text-sm text-slate-500 mt-1">Set individual daily call targets for each agent. Only calls with discussion notes count.</p>
                </div>
              </div>
                    ...
                    {editTarget?.id === agent.id ? (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <button onClick={() => { setEditTarget(agent); setTargetVal(String(agent.daily_target)) }}
                          className="px-3 py-2 border border-slate-200 rounded-lg text-sm font-semibold text-slate-600 hover:bg-slate-50 hover:border-indigo-300 hover:text-indigo-600 transition-colors">
                          ✏️ Edit
                        </button>
                      </div>
                    )}
                    ...
                    return (
                      <div key={i} className="flex items-center gap-4 p-3 bg-slate-50 rounded-xl border border-slate-100">
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0">
                          ...
                          {p >= 100 && <span title="Target achieved">🎯</span>}
                        </div>
                      </div>
                    )
                  })}
