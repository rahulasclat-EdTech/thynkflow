...
// FIX: axios interceptor returns res.data directly, so r = {success, data:[...]}
// All r.data?.data changed to r?.data, r.data?.data?.id changed to r?.data?.id
...
function NewChatModal({ onClose, onCreated, isAdmin }) {
  ...
  useEffect(() => {
    api.get('/chat/users').then(r => {
      // interceptor returns body directly: {success, data:[...]}
      const list = r?.data || r || []
  ...
  const handleCreate = async () => {
    ...
    try {
      if (mode === 'direct') {
        ...
        // interceptor: r = {success, data:{id:X}}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {mode === 'broadcast' ? (
            <div className="space-y-3">
              <p className="text-sm text-slate-500">📢 Creates a broadcast visible to ALL active users.</p>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1 max-h-48 overflow-y-auto">
                {users.length === 0 && <div className="text-center py-6 text-slate-400 text-sm">Loading users…</div>}
                {filtered.map(u => {
                  ...
                  return (
                    <button key={u.id} onClick={() => mode==='direct'?setSelected([u.id]):toggle(u.id)}
                      className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl text-left transition-colors ${sel?'bg-blue-50 border border-blue-200':'hover:bg-slate-50'}`}>
                      ...
                      {sel && <span className="text-blue-600 text-sm">✓</span>}
                    </button>
                  )
                })}
              </div>
  ...
  return (
    <div className={`flex gap-2 mb-3 group ${isMine?'flex-row-reverse':'flex-row'}`}
      ...
      <div className={`max-w-[72%] ${isMine?'items-end':'items-start'} flex flex-col`}>
        {showName && !isMine && <span className="text-xs font-semibold text-slate-500 mb-1 ml-1">{msg.sender_name}</span>}
        ...
        <div className={`flex items-center gap-1.5 mt-0.5 ${isMine?'flex-row-reverse':'flex-row'}`}>
          ...
          {isMine && hovered && (
            <button onClick={() => onDelete(msg.id)} className="text-xs text-red-400 hover:text-red-600 opacity-0 group-hover:opacity-100 transition-opacity">Delete</button>
          )}
        </div>
      </div>
    </div>
...
export default function ChatPage() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // interceptor: r = {success, data:{...msg}}
      const newMsg = r?.data
      if (newMsg) {
        ...
        <div className="p-4 border-b border-slate-100">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => setShowNew(true)}
              className="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center text-white hover:bg-blue-700 transition-colors">✎</button>
          </div>
          <input value={search} onChange={e => setSearch(e.target.value)}
              ...
              <button key={conv.id} onClick={() => setActiveConvId(conv.id)}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-slate-400 flex-shrink-0 ml-2">{formatTime(conv.last_message_at)}</span>
                  </div>
        ...
        <div className="flex-1 flex flex-col">
          <div className="flex items-center gap-3 px-6 py-4 border-b border-slate-100 bg-white">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">{activeConv?TYPE_LABELS[activeConv.type]:''} · {(activeConv?.members||[]).length} members</p>
            </div>
          </div>
          ...
          <div className="px-6 py-4 border-t border-slate-100 bg-white">
            <div className="flex items-end gap-3">
              <label className="flex-shrink-0 w-9 h-9 rounded-xl bg-slate-100 flex items-center justify-center cursor-pointer hover:bg-slate-200 transition-colors">
              ...
              <textarea value={text} onChange={e => setText(e.target.value)} onKeyDown={handleKeyDown}
                ...
                rows={1} style={{ resize:'none', minHeight:'40px', maxHeight:'120px' }}
