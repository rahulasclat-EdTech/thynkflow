// =============================================================
//  ThynkFlow — EmailPage.jsx  (NEW PAGE)
//  web-admin/src/pages/EmailPage.jsx
//
//  Features:
//  - Compose & send email to any lead
//  - Email templates with variable filling
//  - Full sent history with status
//  - Bulk email to multiple leads
//  - Add to App.jsx: import EmailPage + route path="email"
// =============================================================
...
export default function EmailPage() {
  ...
  const [tab, setTab] = useState('compose') // compose | history | templates | bulk
  ...
  // ── lead search ───────────────────────────────────────────
  const searchLeads = async (q, setBulk = false) => {
    if (!q || q.length < 2) { setBulk ? setBulkResults([]) : setLeadResults([]); return }
    try {
      const res = await api.get(`/leads?search=${q}&per_page=10`)
      const rows = Array.isArray(res.data) ? res.data : (res.data?.data || [])
      setBulk ? setBulkResults(rows) : setLeadResults(rows)
    } catch {}
  }
  ...
  const applyTemplate = (tpl, leadName = '', isBulk = false) => {
    ...
    if (isBulk) {
      setBulkTemplate(tpl)
      setBulkSubject(fill(tpl.subject))
      setBulkBody(fill(tpl.body))
    } else {
  ...
  const sendBulk = async () => {
    ...
    setBulkResult(null)
    try {
      const res = await api.post('/emails/bulk', {
        lead_ids:    bulkLeads.map(l => l.id),
        subject:     bulkSubject,
        body:        bulkBody,
        template_id: bulkTemplate?.id || null,
      })
      setBulkResult(res.data?.data)
      toast.success(`Sent ${res.data?.data?.sent} emails`)
    } catch (err) {
  ...
  const testSMTP = async () => {
    try {
      ...
      toast.success(res.data?.message || 'SMTP OK')
    } catch (err) { toast.error(err.message) }
  }
    ...
    <div className="p-4 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-3 gap-6">
          ...
          <div className="space-y-3">
            ...
            {templates.map(t => (
              <button key={t.id} onClick={() => applyTemplate(t, selectedLead?.contact_name || selectedLead?.name || '')}
                className={`w-full text-left p-3 rounded-xl border text-sm transition-all
                  ${selectedTemplate?.id === t.id ? 'border-indigo-400 bg-indigo-50' : 'border-gray-200 hover:border-indigo-200 hover:bg-gray-50'}
            ...
            <button onClick={() => setTab('templates')}
              className="w-full text-xs text-indigo-600 py-2 border border-dashed border-indigo-200 rounded-xl hover:bg-indigo-50">
              + Manage Templates
            </button>
          </div>
          ...
          <div className="col-span-2 space-y-4">
              ...
              {selectedLead ? (
                <div className="flex items-center gap-3 p-3 bg-indigo-50 border border-indigo-200 rounded-xl">
                  ...
                  <button onClick={() => { setSelectedLead(null); setLeadSearch('') }}
                    className="text-indigo-400 hover:text-indigo-700 text-lg">×</button>
                </div>
              ) : (
                <>
            ...
            <div>
              ...
              <textarea value={body} onChange={e => setBody(e.target.value)}
                rows={12} placeholder="Write your email here…"
      ...
      {tab === 'bulk' && (
        <div className="grid grid-cols-3 gap-6">
          ...
          {/* Left: template picker + lead list */}
          <div className="space-y-4">
            <div>
              <h3 className="text-sm font-semibold text-gray-700 mb-2">Choose Template</h3>
              {templates.map(t => (
                <button key={t.id} onClick={() => applyTemplate(t, '', true)}
                  className={`w-full text-left p-3 rounded-xl border text-sm mb-2 transition-all
                    ${bulkTemplate?.id === t.id ? 'border-indigo-400 bg-indigo-50' : 'border-gray-200 hover:border-indigo-200'}`}>
                  <p className="font-semibold">{t.name}</p>
            ...
            <div>
              <h3 className="text-sm font-semibold text-gray-700 mb-2">
                Recipients ({bulkLeads.length})
              </h3>
              <input value={bulkSearch}
                onChange={e => { setBulkSearch(e.target.value); searchLeads(e.target.value, true) }}
              ...
              <div className="space-y-1 max-h-48 overflow-y-auto">
                {bulkLeads.map(l => (
                  <div key={l.id} className="flex items-center justify-between bg-gray-50 rounded-lg px-3 py-1.5">
                    ...
                    <button onClick={() => setBulkLeads(prev => prev.filter(x => x.id !== l.id))}
          ...
          <div className="col-span-2 space-y-4">
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-700">
              <strong>Template variables:</strong> Use <code className="bg-amber-100 px-1 rounded">{'{{lead_name}}'}</code> for the lead's name,
              <code className="bg-amber-100 px-1 rounded ml-1">{'{{agent_name}}'}</code> for your name.
              These are auto-filled for each lead.
            </div>
            ...
            <div>
              ...
              <textarea value={bulkBody} onChange={e => setBulkBody(e.target.value)}
                rows={12} placeholder="Dear {{lead_name}},&#10;&#10;Write your message here…"
                className="w-full border rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 resize-none font-mono" />
            </div>
                  ...
                  {history.map(email => (
                    <tr key={email.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 max-w-xs">
                        <p className="truncate text-gray-800">{email.subject}</p>
                      </td>
